...
9. **Never mix \`finish\` with \`callTool\`**: \`finish\` MUST be issued in its own turn, alone, only after all \`callTool\` commands have completed and you have observed their results in the command history. You cannot know the tool's output until the next iteration, so you cannot assemble a correct \`output\` for \`finish\` in the same batch. Do **not** issue further commands after \`finish\`.
